...
class MicroversionsController(wsgi.Controller):
    ...
    def index(self, req):
        # NOTE(stephenfin): We have to pass a made up max_version because the
        # param defaults to the current max version (which is a 2.x version)
        if api_version_request.is_supported(req, '3.0', '3.99'):
...
class MicroversionsController2(wsgi.Controller):
    ...
    def index(self, req):
        # NOTE(stephenfin): We have to pass max_version because the param
        # defaults to the current max version (which is a 2.x version)
        if api_version_request.is_supported(req, '2.5', '3.1'):
