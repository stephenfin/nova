...
class SingleCellSimple(fixtures.Fixture):
    ...
    def _fake_target_cell(self, context, target_cell):
        ...
        if target_cell:
            context.cell_uuid = getattr(target_cell, 'uuid', uuids.cell1)
        else:
    ...
    def _fake_set_target_cell(self, context, cell_mapping):
        # Just do something simple and set/unset the cell_uuid on the context.
        if cell_mapping:
            context.cell_uuid = getattr(cell_mapping, 'uuid', uuids.cell1)
        else:
            context.cell_uuid = None
...
class OSAPIFixture(fixtures.Fixture):
    ...
    def setUp(self):
        ...
        # we use hardcoded IDs here since these affect our API samples
        self.api = client.TestOpenStackClient(
            'fake',
            'a16a7b90-3330-4e1e-a91b-b6277482fc7f',
            base_url,
            project_id=self.project_id,
            roles=['reader', 'member'])
        self.alternative_api = client.TestOpenStackClient(
            'fake',
            '88c1c636-3520-4ed1-bc78-e66c506769db',
            base_url,
            project_id=self.project_id,
            roles=['reader', 'member'])
        self.admin_api = client.TestOpenStackClient(
            'admin',
            '1debf6db-444d-4ac8-9ed8-09f618a7bc01',
            base_url,
            project_id=self.project_id,
            roles=['reader', 'member', 'admin'])
        self.alternative_admin_api = client.TestOpenStackClient(
            'admin',
            '328faf6d-9aff-4455-adad-932c6409d807',
            base_url,
            project_id=self.project_id,
            roles=['reader', 'member', 'admin'])
        self.reader_api = client.TestOpenStackClient(
            'reader',
            '728d7977-c606-4b6d-b2d6-f2c935815915',
            base_url,
            project_id=self.project_id, roles=['reader'])
        self.other_api = client.TestOpenStackClient(
            'other',
            'd55c468a-6e52-4e72-8850-93903fc98b8e',
            base_url,
            project_id=self.project_id,
            roles=['other'])
    ...
    def _stub_keystone(self):
        ...
        def fake_ctx(env, **kwargs):
            # these are not real headers used in keystoneauth (at least, not
            # anymore): they are generated by TestOpenStackClient
            user_id = env['HTTP_X_USER_ID']
            username = env['HTTP_X_AUTH_USER']
